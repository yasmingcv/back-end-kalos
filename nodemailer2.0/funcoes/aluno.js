/*****************************************************************************************
 * Objetivo: Arquivo responsável pelo envio de emails do aluno
 * Data: 28/09/2023
...
const esqueciASenha = async function (dados) {
    var aluno = await alunoDAO.selectAlunoByEmail(dados.email)

    //Verifica se o email é cadastrado na tabela de alunos
    if (aluno != false) {
        ...
        if (mailSent) {
            await alunoDAO.updateTokenAndExpiresByEmail(dados.email, token, expires)
...
const verificarToken = async function (dados) {
    if (dados.token == undefined || dados.token == null ||
        dados.email == undefined || dados.email == null || !isNaN(dados.email)) {
        return message.ERROR_REQUIRED_FIELDS

    } else {
        let rsAluno = await alunoDAO.selectAlunoByTokenAndEmail(dados.email, dados.token)

        if (rsAluno) {
            const timeZone = 'America/Sao_Paulo'
            const dataAtual = DateTime.now().setZone(timeZone).toFormat('yyyy-MM-dd HH:mm:ss')

            const dataExpiracao = new Date(rsAluno[0].expiracao_token).toISOString().replace('T', ' ').replace(/\.\d{3}Z$/, '')
